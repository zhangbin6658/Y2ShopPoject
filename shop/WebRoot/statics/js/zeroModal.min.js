		...
		(
				...
				function(a) {
					...
					function x(O, E) {
						...
						y = G.pop();
						if (y) {
							throw new Error('Unclosed section "' + y[1]
									+ '" at ' + A.pos)
						}
						return w(c(F))
					}
					...
					p.prototype.renderTokens = function(G, y, E, I) {
						...
						for (var D = 0, F = G.length; D < F; ++D) {
							...
							case "#":
								...
								if (l(H)) {
									for (var B = 0, J = H.length; B < J; ++B) {
										C += this.renderTokens(A[4], y
												.push(H[B]), E, I)
									}
								} else {
									if (typeof H === "object"
											|| typeof H === "string") {
										C += this.renderTokens(A[4], y.push(H),
												E, I)
									} else {
										if (b(H)) {
											if (typeof I !== "string") {
											...
											H = H.call(y.view, I.slice(A[3],
													A[5]), z);
											if (H != null) {
												C += H
											}
										} else {
											C += this.renderTokens(A[4], y, E,
													I)
										}
									}
								}
							...
							case ">":
								...
								H = b(E) ? E(A[1]) : E[A[1]];
								if (H != null) {
									C += this.renderTokens(this.parse(H), y, E,
											H)
								}
								break;
							case "&":
		...
		((function(f, h) {
			...
			z.loading = function(E) {
				...
				if (E === 1 || E === 2) {
					...
					f("body").append(
							'<div role="zeromodal-loading" zero-unique-loading="'
									+ H.unique + '" class="' + F
									+ '" style="z-index:' + t + ";top:" + G
									+ 'px;"></div>')
				} else {
					if (k([ 3, 4, 5, 6 ], E)) {
						var B = {};
						...
						var C = '<div role="zeromodal-loading" zero-unique-loading="'
								+ H.unique
								+ '" class="'
								+ B.className
								+ '" style="z-index:'
			...
			z.progress = function(H, B) {
				...
				var C = '<div zero-unique-loading="' + E.unique + '" class="'
						+ I.className + '" style="z-index:' + t
						+ ";left:46%;top:" + J + 'px;">';
				for (var G = 0; G < I.containerCount; G++) {
					C += "  <div></div>"
				}
				C += "  </div>";
				C += '  <div zero-unique-loading="'
						+ E.unique
						+ '" class="zeromodal-progress-content" style="z-index:'
						+ t + ";top:" + (J + 64)
						+ 'px;"><span id="progess_content_' + E.unique
						+ '"></span></div>';
				C += "";
				f("body").append(C);
				var F = 0;
			...
			z.progress_old = function(D) {
				...
				var C = '<div class="zeromodal-progress" style="top:' + J
						+ "px;left:" + K + "px;z-index:" + t + '">';
				C += '      <div zeromodal-progress-bar="'
						+ G.unique
						+ '" class="zeromodal-progress-bar" style="width: 0%; background: #92c26a;">';
				C += '          <span class="zeromodal-progress-icon zeromodal-fa zeromodal-fa-check" style="border-color:#92c26a; color:#92c26a;"><div zeromodal-progress-val="'
						+ G.unique
						+ '" class="zeromodal-progress-val">&nbsp;0%</div></span>';
				C += "      </div>";
				C += "  </div>";
				f("body").append(C);
				var B = 0;
				...
				var E = setInterval(
						function() {
							...
							if (B >= 100) {
								F
										.html('<span class="line tip"></span><span class="line long"></span>');
								clearInterval(E)
							}
						}, H * 100);
				return G.unique
			...
			function n(D) {
				...
				var L = ((C - F) / 2) + "px";
				var K = (f(window).scrollTop() + Math
						.ceil((f(window).height() - H) / 3))
						+ "px";
				if (D.top !== h) {
					K = D.top
				}
				...
				f('[zero-max-unique="' + D.unique + '"]').click(
						function() {
							var M = f('[zeromodal-unqiue-header="' + D.unique
									+ '"]');
							f('[zero-title-unique="' + D.unique + '"]')
									.removeClass("modal-title-min");
							f('[zero-unique-body="' + D.unique + '"]').show();
							f('[zero-unique-container="' + D.unique + '"]')
									.removeClass("zeromodal-fixed");
							if (M.attr("zero-status") !== "2") {
								i(m[f(this).attr("zero-max-unique")], "90%",
										"85%");
				...
				f('[zero-min-unique="' + D.unique + '"]')
						.click(
								function() {
									var O = f('[zeromodal-unqiue-header="'
											+ D.unique + '"]');
									var N = f('[zero-unique-container="'
											+ D.unique + '"]');
									var P = f('[zero-unique-body="' + D.unique
											+ '"]');
									if (O.attr("zero-status") !== "0") {
										P.hide();
										N.css("height", "22px").css("width",
												"220px").addClass(
												"zeromodal-fixed");
										f(
												'[zero-title-unique="'
														+ D.unique + '"]')
												.addClass("modal-title-min");
										if (D.minPosition !== h
												&& D.minPosition !== "") {
											var M = D.minPosition.split(",");
											if (M.length === 2) {
												N.css("left", M[0] + "px").css(
														"top",
														(parseInt(M[1]) + f(
																window)
																.scrollTop())
				...
				if (D.forbidBodyScroll) {
					var I = f("body");
					if (D.forbidBodyScrollOffset !== h && r
							&& D.forbidBodyScrollClass !== h) {
				...
				if (G.ok || G.cancel
					...
					if (G.buttons !== h && G.buttons.length > 0) {
						for (var F = 0; F < G.buttons.length; F++) {
							...
							f('[zeromodal-btn-container="' + G.unique + '"]')
									.append(E)
						}
					} else {
						if (G.ok) {
							var H = f('<button zeromodal-btn-ok="'
									+ G.unique
									+ '" class="zeromodal-btn zeromodal-btn-primary">'
									+ G.okTitle + "</button>");
							f('[zeromodal-btn-container="' + G.unique + '"]')
									.append(H);
							H.click(function() {
						...
						if (G.cancel) {
							var I = f('<button zeromodal-btn-cancel="'
									+ G.unique
									+ '" class="zeromodal-btn zeromodal-btn-default">'
									+ G.cancelTitle + "</button>");
							f('[zeromodal-btn-container="' + G.unique + '"]')
									.append(I);
							I.click(function() {
			...
			function q(C) {
				if (typeof C === "undefined"
						|| typeof C.cancelTitle === "undefined") {
					C.cancelTitle = "取消"
				}
				...
				f('[zero-unique-body="' + D.unique + '"]').removeClass(
			...
			function i(D, C, I) {
				...
				f('[zero-unique-overlay="' + D.unique + '"]').css("width",
						f(document).width() + "px").css("height",
						f(document).height() + "px");
				...
				E.css("width", F + "px").css("height", H + "px").css("left", K)
						.css("top", J)
			}
			function s(G) {
				var C = f(window).width();
				...
				var I = f(window).scrollTop()
						+ Math.ceil((f(window).height() - F) / 3);
				D.css("left", H + "px").css("top", I + "px")
			}
			function p(D) {
				var C = f('[zeromodal-unqiue-header="' + D.unique + '"]')
						.height();
				var E = (D.ok || D.cancel || (D.buttons !== h && D.buttons.length > 0)) ? 60
						: 0;
				var B = f('[zero-unique-container="' + D.unique + '"]')
						.height()
						- C - 10 - E;
			...
			function A(H, E) {
				...
				G.onmousedown = function() {
					...
					I = setInterval(function() {
						if (I && F) {
							var M = C - K;
							var L = B - J;
							f(".zeromodal-container")
									.css("width", M + 3 + "px").css("height",
											L + 3 + "px")
						}
					}, 5)
				};
				document.onmouseup = function() {
					...
					s(E);
					p(E);
					if (E.resizeAfterFn !== h
							&& typeof E.resizeAfterFn === "function") {
						E.resizeAfterFn(E)
					}
				}
			}
			...
			a.prototype = {
				initialize : function(C, B) {
					...
					this.maxTop = Math.max(this.maxContainer.clientHeight,
							this.maxContainer.scrollHeight)
							- this.drag.offsetHeight;
					this.maxLeft = Math.max(this.maxContainer.clientWidth,
					...
					this.changeLayout();
					this.addHandler(this.handle, "mousedown", this.bind(this,
							this.startDrag))
				},
				...
				addHandler : function(C, D, B) {
					return C.addEventListener ? C.addEventListener(D, B, false)
							: C.attachEvent("on" + D, B)
				},
				removeHandler : function(C, D, B) {
					return C.removeEventListener ? C.removeEventListener(D, B,
							false) : C.detachEvent("on" + D, B)
				},
				bind : function(C, B) {
					return function() {
